...
def add_heteroscedastic_gnoise(image, device, sigma_1_range=(5e-3, 5e-2), sigma_2_range=(1e-3, 1e-2)):
    """
...
def add_noise(image, shot_noise=0.01, read_noise=0.0005, use_cuda=True):
    ...
    if use_cuda:
        shot_noise = torch.Tensor(shot_noise).cuda()
        read_noise = torch.Tensor(read_noise).cuda()
    variance = image * shot_noise + read_noise
    scale = torch.sqrt(variance)
    if use_cuda:
        distribution = dist.normal.Normal(torch.Tensor([0.0]).cuda(), scale)
    else:
