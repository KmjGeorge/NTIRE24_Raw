...
def main():
    ...
    if mode == 'single':
        opt['input_folder'] = 'F:\Datasets\DSLR\\train_raws'
        opt['save_folder'] = 'D:\Datasets\DSLR\subimages_new\\512'
        opt['crop_sz'] = 512  # the size of each sub-image
        opt['step'] = 384  # step of the sliding crop window
...
def worker(path, opt):
    ...
    save_prob = opt['save_prob']
    img_name = osp.basename(path)
    raw_img, raw_max = load_data(path)
    n_channels = len(raw_img.shape)
    if n_channels == 2:
        h, w = raw_img.shape
    elif n_channels == 3:
        h, w, c = raw_img.shape
    ...
    for x in h_space:
        for y in w_space:
            index += 1
            if n_channels == 2:
                crop_img = raw_img[x:x + crop_sz, y:y + crop_sz]
            else:
                crop_img = raw_img[x:x + crop_sz, y:y + crop_sz, :]
            ...
            if np.random.rand() <= save_prob:
                if save_npz:
                    np.savez(osp.join(opt['save_folder'], img_name.replace('.npz', '_s{:03d}.npz'.format(index))),
                             raw=crop_img, max_val=raw_max)
                else:
                    np.save(osp.join(opt['save_folder'], img_name.replace('.npz', '_s{:03d}.npy'.format(index))), crop_img)
